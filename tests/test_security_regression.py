...
```"""
    ...
    def test_render_report_from_draft_v3_uses_data_driven_mermaid_and_table_layout(self):
        ...
        try:
            ...
            self.assertIn("| 编号 | 优先级 | 需求项 | 描述 |", report)
            self.assertIn("| 编号 | 方案建议 | 说明 | Owner | 时间计划 | 验收指标 |", report)
            self.assertIn("| 编号 | 风险项 | 影响 | 缓解措施 |", report)
            self.assertIn("| 编号 | 行动项 | Owner | 时间计划 | 验收指标 |", report)
            self.assertIn("| 编号 | 未决问题 | 原因 | 影响 | 建议补问 |", report)
            self.assertNotIn("| 编号 | 优先级 | 需求项 | 描述 | 证据 |", report)
            self.assertNotIn("| 编号 | 方案建议 | 说明 | Owner | 时间计划 | 验收指标 | 证据 |", report)
            self.assertNotIn("| 编号 | 风险项 | 影响 | 缓解措施 | 证据 |", report)
            self.assertNotIn("| 编号 | 行动项 | Owner | 时间计划 | 验收指标 | 证据 |", report)
            self.assertNotIn("| 编号 | 未决问题 | 原因 | 影响 | 建议补问 | 证据 |", report)
            self.assertNotIn("Q1", report)
            self.assertNotIn("Q6", report)
            self.assertIn("访谈输入", report)
            self.assertIn("title Priority Matrix", report)
            self.assertIn("x-axis Low --> High", report)
            self.assertIn("quadrant-1 Do First", report)
            self.assertNotIn("x-axis 紧急程度", report)
            self.assertNotIn("quadrant-1 立即执行", report)
